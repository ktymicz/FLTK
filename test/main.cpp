...
void arrow(Graph_lib::Point p1, Graph_lib::Point p2)
{
    using namespace Graph_lib;
    Point pWin{100,100};
    Simple_window win{pWin ,width_window-1000,hight_window-500,"My window"};
    ...
    // tan (ang) = |x| / |y|
    // tan^-1 (tan (ang)) = tan^-1 (|x| / |y|)
    double angle_rad = atan(a); //* (180/PI);
    double angle_ = atan(a) * (180/PI);
    // b = y - a*x
    double b = p1.y - p1.x*a;

    // arrow is on y line , p1 and p2 on y line

    // d - difference between p2.y and yd
    // sin angle_ = d / hight; //
    // calculate d
    double d, hight;
    // the hight of arrow's triangle 10 pixels
    hight = 10;
    //if(angle_>0)
    d = hight * sin(angle_rad);

    if( d - int(d) >= 0.4 || (d < 1 && d>0))
        d= int(d) + 1;
    else if( d<0 && (-(d - int(d)) >= 0.4  || d>-1))
       d = int(d) - 1;



    int yd, xd;
    if(d>0 && p2.y-p1.y>0 || a<0 && d<0 && p2.y-p1.y<0)//&& p2.x-p1.x>0)
         yd = p2.y - d;
    else
        yd = p2.y + d;
    ...
    // *****************************
    // base of arrow - 10 pixels
    // y2 = a2*x2 + b2
    // the perpendicular line to y
    double a2 = -1/a;
    double b2 = yd - a2*xd;
    //  the distance from y line, it's middle of arrow, - 5
    // it's also triangle's diagonal
    int diagonal = 3;
    double angle_2 = 90 - abs(angle_rad);
    // sin angle_2  =  distance / diagonal
    double dis;
    //if(angle_2>0 && angle_2 < 90)
        dis = diagonal *  sin(angle_2);
    //else
      //  dis = abs(diagonal *  cos(angle_2));
    ...
    p1_Ar.y = yd - dis;
    p1_Ar.x = (p1_Ar.y-b2) / a2;
    p2_Ar.y = yd + dis;
    p2_Ar.x = (p2_Ar.y-b2) / a2;

    // y = ax+b


    std::cout << " a: "<<a<<" angle: " << angle_rad << " rad, angle: "
    << (angle_rad*180/(double)PI) << " s, b: " <<  b << '\n';
    ...
    << "p2_Ar.x:" << p2_Ar.x << '\n'
    << "p2_Ar.y:" << p2_Ar.y << '\n';
    Graph_lib::Open_polyline arrow_line;
    arrow_line.add(p1);
    arrow_line.add(p2);
    arrow_line.set_color(Color::black);
    Graph_lib::Polygon arrow_tip;
    arrow_tip.add(p1_Ar);
    arrow_tip.add(p2_Ar);
    arrow_tip.add(p2);
    arrow_tip.set_fill_color(Color::black);
    arrow_tip.set_color(Color::black);
    win.attach(arrow_line);
	win.attach(arrow_tip);
    win.wait_for_button();
...
//using namespace Graph_lib; // our graphics facilities are in Graph_lib
 ...
 //tooBig();// ex 6
   ...
   // arrow(Point{1,1}, Point{3,3});

    // ex04 drawTicTacToeBoard(Graph_lib::Simple_window & win, int side_size, Point startP)
     // ex10(win);
    //ex12(8,5,2,2,40);
